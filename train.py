...
wandb.init(project="test-project", entity="sciml-leeds")
...
def train_model(model, epochs, opt, loss, batch_size):
    ...
    # create dataset
    data_train = PlumeSegmentationDataset(
        datadir='data/train',
        seglabeldir='data/train/segmentation_labels', mult=1, transform=None)
    data_val = PlumeSegmentationDataset(
        datadir='data/val',
        seglabeldir='data/val/segmentation_labels', mult=1, transform=None)
    ...
    train_dl = DataLoader(data_train, batch_size=batch_size, num_workers=0,
                          pin_memory=True, sampler=train_sampler)
    val_dl = DataLoader(data_val, batch_size=batch_size, num_workers=0,
    ...
    for epoch in range(epochs):
        ...
        for i, batch in progress:
            ...
            for j in range(y.shape[0]):
                z = jaccard_score(y[j].flatten().cpu().detach().numpy(),
                          output_binary[j][0].flatten(), zero_division=0)
        ...
        wandb.log({"train loss": train_loss_total/(i+1),
                   "val loss": val_loss_total/(j+1),
                   "train iou": np.average(train_ious),
                   "val iou": np.average(val_ious),
                   "train acc": train_acc_total/(i+1),
                   "val acc": val_acc_total/(i+1)})
        wandb.watch(model)
